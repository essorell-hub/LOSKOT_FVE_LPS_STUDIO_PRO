...
export const FVE_FUNCTIONAL_DC_PROTECTION_PLANNER_V346_VERSION = 'v346-fve-functional-dc-protection-planner-v346';

export function createFveFunctionalDcProtectionPlannerV346(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_DC_PROTECTION_PLANNER_V346_VERSION,
    versionNumber: 346,
    phaseId: 'V341-V360',
    phaseTitle: 'FVE functional core',
    functionalDomain: 'fve-functional',
    functionalItem: 'DcProtectionPlanner',
    status: 'V346_READY',
    routeKey,
    project: {
...
export function validateFveFunctionalDcProtectionPlannerV346(model = createFveFunctionalDcProtectionPlannerV346()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_DC_PROTECTION_PLANNER_V346_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 346) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 346) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
