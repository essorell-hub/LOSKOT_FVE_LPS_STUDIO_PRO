...
export const PROJECT_RUNTIME_LPS_MODEL_V268_VERSION = 'v268-project-runtime-lps-model-v268';

export function createProjectRuntimeLpsModelV268(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_LPS_MODEL_V268_VERSION,
    versionNumber: 268,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeLpsModelV268(model = createProjectRuntimeLpsModelV268()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_LPS_MODEL_V268_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 268) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 268) errors.push('Runtime step invalid.');
