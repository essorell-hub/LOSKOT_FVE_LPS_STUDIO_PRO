...
export const V75_FULL_APP_CANDIDATE_VERSION = "v75-full-app-candidate";

const MODULE_NAME = "fullAppCandidateRuntime";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V75_FULL_APP_CANDIDATE_VERSION,
...
export function createFullAppCandidateRuntime(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V75_FULL_APP_CANDIDATE_VERSION,
      purpose: "První FULL APP Candidate: stabilizovaný integrační kandidát před další PRO stabilizací.",
      classicProUnchanged: true
    });
  }
...
export function safeFullAppCandidateRuntime(options = {}) {
  try {
    return createFullAppCandidateRuntime(options);
  } catch (error) {
    return {
      version: V75_FULL_APP_CANDIDATE_VERSION,
      classicProUnchanged: true,
