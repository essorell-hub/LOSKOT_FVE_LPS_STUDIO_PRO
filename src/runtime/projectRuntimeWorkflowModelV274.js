...
export const PROJECT_RUNTIME_WORKFLOW_MODEL_V274_VERSION = 'v274-project-runtime-workflow-model-v274';

export function createProjectRuntimeWorkflowModelV274(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_WORKFLOW_MODEL_V274_VERSION,
    versionNumber: 274,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeWorkflowModelV274(model = createProjectRuntimeWorkflowModelV274()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_WORKFLOW_MODEL_V274_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 274) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 274) errors.push('Runtime step invalid.');
