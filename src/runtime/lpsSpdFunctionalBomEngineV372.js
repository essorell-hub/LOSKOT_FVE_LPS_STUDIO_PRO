...
export const LPS_SPD_FUNCTIONAL_BOM_ENGINE_V372_VERSION = 'v372-lps-spd-functional-bom-engine-v372';

export function createLpsSpdFunctionalBomEngineV372(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_BOM_ENGINE_V372_VERSION,
    versionNumber: 372,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'BomEngine',
    status: 'V372_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalBomEngineV372(model = createLpsSpdFunctionalBomEngineV372()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_BOM_ENGINE_V372_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 372) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 372) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
