...
export const DOCUMENTS_EXPORT_FUNCTIONAL_EXPORT_ADAPTER_V397_VERSION = 'v397-documents-export-functional-export-adapter-v397';

export function createDocumentsExportFunctionalExportAdapterV397(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_EXPORT_ADAPTER_V397_VERSION,
    versionNumber: 397,
    phaseId: 'V381-V400',
    phaseTitle: 'documents export functional core',
    functionalDomain: 'documents-export-functional',
    functionalItem: 'ExportAdapter',
    status: 'V397_READY',
    routeKey,
    project: {
...
export function validateDocumentsExportFunctionalExportAdapterV397(model = createDocumentsExportFunctionalExportAdapterV397()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_EXPORT_ADAPTER_V397_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 397) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 397) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
