...
export const DOCUMENTS_EXPORT_FUNCTIONAL_DESIGN_CORE_V381_VERSION = 'v381-documents-export-functional-design-core-v381';

export function createDocumentsExportFunctionalDesignCoreV381(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_DESIGN_CORE_V381_VERSION,
    versionNumber: 381,
    phaseId: 'V381-V400',
...
export function validateDocumentsExportFunctionalDesignCoreV381(model = createDocumentsExportFunctionalDesignCoreV381()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_DESIGN_CORE_V381_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 381) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 381) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
