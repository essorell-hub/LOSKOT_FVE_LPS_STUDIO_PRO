...
export const PROJECT_RUNTIME_QA_MODEL_V273_VERSION = 'v273-project-runtime-qa-model-v273';

export function createProjectRuntimeQaModelV273(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_QA_MODEL_V273_VERSION,
    versionNumber: 273,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeQaModelV273(model = createProjectRuntimeQaModelV273()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_QA_MODEL_V273_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 273) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 273) errors.push('Runtime step invalid.');
