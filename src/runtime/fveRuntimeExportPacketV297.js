...
export const FVE_RUNTIME_EXPORT_PACKET_V297_VERSION = 'v297-fve-runtime-export-packet-v297';

export function createFveRuntimeExportPacketV297(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_EXPORT_PACKET_V297_VERSION,
    versionNumber: 297,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeExportPacketV297(model = createFveRuntimeExportPacketV297()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_EXPORT_PACKET_V297_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 297) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 297) errors.push('Runtime step invalid.');
