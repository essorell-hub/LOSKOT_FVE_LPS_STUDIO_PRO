...
export const FVE_RUNTIME_BILL_OF_MATERIALS_V294_VERSION = 'v294-fve-runtime-bill-of-materials-v294';

export function createFveRuntimeBillOfMaterialsV294(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_BILL_OF_MATERIALS_V294_VERSION,
    versionNumber: 294,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeBillOfMaterialsV294(model = createFveRuntimeBillOfMaterialsV294()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_BILL_OF_MATERIALS_V294_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 294) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 294) errors.push('Runtime step invalid.');
