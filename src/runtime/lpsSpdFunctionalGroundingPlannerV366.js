...
export const LPS_SPD_FUNCTIONAL_GROUNDING_PLANNER_V366_VERSION = 'v366-lps-spd-functional-grounding-planner-v366';

export function createLpsSpdFunctionalGroundingPlannerV366(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_GROUNDING_PLANNER_V366_VERSION,
    versionNumber: 366,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'GroundingPlanner',
    status: 'V366_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalGroundingPlannerV366(model = createLpsSpdFunctionalGroundingPlannerV366()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_GROUNDING_PLANNER_V366_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 366) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 366) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
