...
export const FVE_RUNTIME_INVERTER_MODEL_V285_VERSION = 'v285-fve-runtime-inverter-model-v285';

export function createFveRuntimeInverterModelV285(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_INVERTER_MODEL_V285_VERSION,
    versionNumber: 285,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeInverterModelV285(model = createFveRuntimeInverterModelV285()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_INVERTER_MODEL_V285_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 285) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 285) errors.push('Runtime step invalid.');
