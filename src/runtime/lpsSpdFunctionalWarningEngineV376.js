...
export const LPS_SPD_FUNCTIONAL_WARNING_ENGINE_V376_VERSION = 'v376-lps-spd-functional-warning-engine-v376';

export function createLpsSpdFunctionalWarningEngineV376(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_WARNING_ENGINE_V376_VERSION,
    versionNumber: 376,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'WarningEngine',
    status: 'V376_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalWarningEngineV376(model = createLpsSpdFunctionalWarningEngineV376()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_WARNING_ENGINE_V376_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 376) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 376) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
