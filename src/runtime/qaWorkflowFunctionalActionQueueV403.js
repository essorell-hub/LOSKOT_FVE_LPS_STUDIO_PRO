...
export const QA_WORKFLOW_FUNCTIONAL_ACTION_QUEUE_V403_VERSION = 'v403-qa-workflow-functional-action-queue-v403';

export function createQaWorkflowFunctionalActionQueueV403(options = {}) {
...
export function validateQaWorkflowFunctionalActionQueueV403(model = createQaWorkflowFunctionalActionQueueV403()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_ACTION_QUEUE_V403_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 403) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 403) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
