...
export const LPS_SPD_RUNTIME_AIR_TERMINATION_MODEL_V303_VERSION = 'v303-lps-spd-runtime-air-termination-model-v303';

export function createLpsSpdRuntimeAirTerminationModelV303(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_AIR_TERMINATION_MODEL_V303_VERSION,
    versionNumber: 303,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeAirTerminationModelV303(model = createLpsSpdRuntimeAirTerminationModelV303()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_AIR_TERMINATION_MODEL_V303_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 303) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 303) errors.push('Runtime step invalid.');
