...
export const PROJECT_RUNTIME_DATA_CONTRACT_V262_VERSION = 'v262-project-runtime-data-contract-v262';

export function createProjectRuntimeDataContractV262(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_DATA_CONTRACT_V262_VERSION,
    versionNumber: 262,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeDataContractV262(model = createProjectRuntimeDataContractV262()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_DATA_CONTRACT_V262_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 262) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 262) errors.push('Runtime step invalid.');
