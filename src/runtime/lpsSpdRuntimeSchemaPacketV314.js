...
export const LPS_SPD_RUNTIME_SCHEMA_PACKET_V314_VERSION = 'v314-lps-spd-runtime-schema-packet-v314';

export function createLpsSpdRuntimeSchemaPacketV314(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_SCHEMA_PACKET_V314_VERSION,
    versionNumber: 314,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeSchemaPacketV314(model = createLpsSpdRuntimeSchemaPacketV314()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_SCHEMA_PACKET_V314_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 314) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 314) errors.push('Runtime step invalid.');
