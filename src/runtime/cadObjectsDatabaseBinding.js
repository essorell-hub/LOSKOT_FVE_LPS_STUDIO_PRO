...
export const V67_CAD_OBJECTS_DATABASE_BINDING_VERSION = "v67-cad-objects-database-binding";

const MODULE_NAME = "cadObjectsDatabaseBinding";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V67_CAD_OBJECTS_DATABASE_BINDING_VERSION,
...
export function createCadObjectsDatabaseBinding(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V67_CAD_OBJECTS_DATABASE_BINDING_VERSION,
      purpose: "Napojení CAD objektů, vrstev a symbolů na databázový/project model.",
      classicProUnchanged: true
    });
  }
...
export function safeCadObjectsDatabaseBinding(options = {}) {
  try {
    return createCadObjectsDatabaseBinding(options);
  } catch (error) {
    return {
      version: V67_CAD_OBJECTS_DATABASE_BINDING_VERSION,
      classicProUnchanged: true,
