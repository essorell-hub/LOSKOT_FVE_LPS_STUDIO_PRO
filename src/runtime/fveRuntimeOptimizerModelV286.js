...
export const FVE_RUNTIME_OPTIMIZER_MODEL_V286_VERSION = 'v286-fve-runtime-optimizer-model-v286';

export function createFveRuntimeOptimizerModelV286(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_OPTIMIZER_MODEL_V286_VERSION,
    versionNumber: 286,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeOptimizerModelV286(model = createFveRuntimeOptimizerModelV286()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_OPTIMIZER_MODEL_V286_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 286) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 286) errors.push('Runtime step invalid.');
