...
export const FVE_RUNTIME_SCHEMA_PACKET_V293_VERSION = 'v293-fve-runtime-schema-packet-v293';

export function createFveRuntimeSchemaPacketV293(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_SCHEMA_PACKET_V293_VERSION,
    versionNumber: 293,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeSchemaPacketV293(model = createFveRuntimeSchemaPacketV293()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_SCHEMA_PACKET_V293_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 293) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 293) errors.push('Runtime step invalid.');
