...
export const FVE_RUNTIME_DOCUMENT_INPUTS_V296_VERSION = 'v296-fve-runtime-document-inputs-v296';

export function createFveRuntimeDocumentInputsV296(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_DOCUMENT_INPUTS_V296_VERSION,
    versionNumber: 296,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeDocumentInputsV296(model = createFveRuntimeDocumentInputsV296()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_DOCUMENT_INPUTS_V296_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 296) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 296) errors.push('Runtime step invalid.');
