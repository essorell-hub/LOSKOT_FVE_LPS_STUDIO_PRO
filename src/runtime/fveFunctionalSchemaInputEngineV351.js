...
export const FVE_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V351_VERSION = 'v351-fve-functional-schema-input-engine-v351';

export function createFveFunctionalSchemaInputEngineV351(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V351_VERSION,
    versionNumber: 351,
    phaseId: 'V341-V360',
...
export function validateFveFunctionalSchemaInputEngineV351(model = createFveFunctionalSchemaInputEngineV351()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V351_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 351) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 351) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
