...
export const PROJECT_RUNTIME_CORE_V261_VERSION = 'v261-project-runtime-core-v261';

export function createProjectRuntimeCoreV261(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_CORE_V261_VERSION,
    versionNumber: 261,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeCoreV261(model = createProjectRuntimeCoreV261()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_CORE_V261_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 261) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 261) errors.push('Runtime step invalid.');
