...
export const V74_FULL_APP_PREVIEW_VERSION = "v74-full-app-preview";

const MODULE_NAME = "fullAppPreviewRuntime";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V74_FULL_APP_PREVIEW_VERSION,
...
export function createFullAppPreviewRuntime(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V74_FULL_APP_PREVIEW_VERSION,
      purpose: "První FULL APP Preview: workflow obrazovky, runtime orchestrace a safe fallback UI.",
      classicProUnchanged: true
    });
  }
...
export function safeFullAppPreviewRuntime(options = {}) {
  try {
    return createFullAppPreviewRuntime(options);
  } catch (error) {
    return {
      version: V74_FULL_APP_PREVIEW_VERSION,
      classicProUnchanged: true,
