...
export const LPS_SPD_RUNTIME_BILL_OF_MATERIALS_V315_VERSION = 'v315-lps-spd-runtime-bill-of-materials-v315';

export function createLpsSpdRuntimeBillOfMaterialsV315(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_BILL_OF_MATERIALS_V315_VERSION,
    versionNumber: 315,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeBillOfMaterialsV315(model = createLpsSpdRuntimeBillOfMaterialsV315()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_BILL_OF_MATERIALS_V315_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 315) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 315) errors.push('Runtime step invalid.');
