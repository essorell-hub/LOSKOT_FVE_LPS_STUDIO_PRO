...
export const DOCUMENTS_EXPORT_RUNTIME_FQ_MAPPING_GUARD_V335_VERSION = 'v335-documents-export-runtime-fq-mapping-guard-v335';

export function createDocumentsExportRuntimeFqMappingGuardV335(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_FQ_MAPPING_GUARD_V335_VERSION,
    versionNumber: 335,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeFqMappingGuardV335(model = createDocumentsExportRuntimeFqMappingGuardV335()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_FQ_MAPPING_GUARD_V335_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 335) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 335) errors.push('Runtime step invalid.');
