...
export const PROJECT_RUNTIME_JOB_MODEL_V264_VERSION = 'v264-project-runtime-job-model-v264';

export function createProjectRuntimeJobModelV264(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_JOB_MODEL_V264_VERSION,
    versionNumber: 264,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeJobModelV264(model = createProjectRuntimeJobModelV264()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_JOB_MODEL_V264_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 264) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 264) errors.push('Runtime step invalid.');
