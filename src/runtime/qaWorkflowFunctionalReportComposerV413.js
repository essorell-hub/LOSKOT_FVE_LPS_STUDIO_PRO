...
export const QA_WORKFLOW_FUNCTIONAL_REPORT_COMPOSER_V413_VERSION = 'v413-qa-workflow-functional-report-composer-v413';

export function createQaWorkflowFunctionalReportComposerV413(options = {}) {
  ...
  const model = {
    modelVersion: QA_WORKFLOW_FUNCTIONAL_REPORT_COMPOSER_V413_VERSION,
    versionNumber: 413,
    phaseId: 'V401-V420',
    phaseTitle: 'QA workflow integration functional core',
    functionalDomain: 'qa-workflow-functional',
    functionalItem: 'ReportComposer',
    status: 'V413_READY',
    routeKey,
    project: {
...
export function validateQaWorkflowFunctionalReportComposerV413(model = createQaWorkflowFunctionalReportComposerV413()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_REPORT_COMPOSER_V413_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 413) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 413) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
