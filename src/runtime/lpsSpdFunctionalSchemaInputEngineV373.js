...
export const LPS_SPD_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V373_VERSION = 'v373-lps-spd-functional-schema-input-engine-v373';

export function createLpsSpdFunctionalSchemaInputEngineV373(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V373_VERSION,
    versionNumber: 373,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'SchemaInputEngine',
    status: 'V373_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalSchemaInputEngineV373(model = createLpsSpdFunctionalSchemaInputEngineV373()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_SCHEMA_INPUT_ENGINE_V373_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 373) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 373) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
