...
export const DOCUMENTS_EXPORT_FUNCTIONAL_RELEASE_GATE_V398_VERSION = 'v398-documents-export-functional-release-gate-v398';

export function createDocumentsExportFunctionalReleaseGateV398(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_RELEASE_GATE_V398_VERSION,
    versionNumber: 398,
    phaseId: 'V381-V400',
...
export function validateDocumentsExportFunctionalReleaseGateV398(model = createDocumentsExportFunctionalReleaseGateV398()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_RELEASE_GATE_V398_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 398) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 398) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
