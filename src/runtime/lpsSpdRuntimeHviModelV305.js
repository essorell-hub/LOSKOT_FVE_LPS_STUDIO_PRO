...
export const LPS_SPD_RUNTIME_HVI_MODEL_V305_VERSION = 'v305-lps-spd-runtime-hvi-model-v305';

export function createLpsSpdRuntimeHviModelV305(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_HVI_MODEL_V305_VERSION,
    versionNumber: 305,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeHviModelV305(model = createLpsSpdRuntimeHviModelV305()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_HVI_MODEL_V305_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 305) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 305) errors.push('Runtime step invalid.');
