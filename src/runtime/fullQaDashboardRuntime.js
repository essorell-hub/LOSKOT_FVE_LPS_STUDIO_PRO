...
export const V72_FULL_QA_DASHBOARD_VERSION = "v72-full-qa-dashboard";

const MODULE_NAME = "fullQaDashboardRuntime";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V72_FULL_QA_DASHBOARD_VERSION,
...
export function createFullQaDashboardRuntime(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V72_FULL_QA_DASHBOARD_VERSION,
      purpose: "Sjednocený QA dashboard pro FVE, CAD, LPS, dokumenty, export a release readiness.",
      classicProUnchanged: true
    });
  }
...
export function safeFullQaDashboardRuntime(options = {}) {
  try {
    return createFullQaDashboardRuntime(options);
  } catch (error) {
    return {
      version: V72_FULL_QA_DASHBOARD_VERSION,
      classicProUnchanged: true,
