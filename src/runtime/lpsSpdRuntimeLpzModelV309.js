...
export const LPS_SPD_RUNTIME_LPZ_MODEL_V309_VERSION = 'v309-lps-spd-runtime-lpz-model-v309';

export function createLpsSpdRuntimeLpzModelV309(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_LPZ_MODEL_V309_VERSION,
    versionNumber: 309,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeLpzModelV309(model = createLpsSpdRuntimeLpzModelV309()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_LPZ_MODEL_V309_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 309) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 309) errors.push('Runtime step invalid.');
