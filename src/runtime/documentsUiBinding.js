...
export const V70_DOCUMENTS_UI_BINDING_VERSION = "v70-documents-ui-binding";

const MODULE_NAME = "documentsUiBinding";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V70_DOCUMENTS_UI_BINDING_VERSION,
...
export function createDocumentsUiBinding(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V70_DOCUMENTS_UI_BINDING_VERSION,
      purpose: "Napojení dokumentů/reportů/export QA do UI a projektového workflow.",
      classicProUnchanged: true
    });
  }
...
export function safeDocumentsUiBinding(options = {}) {
  try {
    return createDocumentsUiBinding(options);
  } catch (error) {
    return {
      version: V70_DOCUMENTS_UI_BINDING_VERSION,
      classicProUnchanged: true,
