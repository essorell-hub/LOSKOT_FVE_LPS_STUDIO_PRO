...
export const LPS_SPD_FUNCTIONAL_RELEASE_GATE_V378_VERSION = 'v378-lps-spd-functional-release-gate-v378';

export function createLpsSpdFunctionalReleaseGateV378(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_RELEASE_GATE_V378_VERSION,
    versionNumber: 378,
    phaseId: 'V361-V380',
...
export function validateLpsSpdFunctionalReleaseGateV378(model = createLpsSpdFunctionalReleaseGateV378()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_RELEASE_GATE_V378_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 378) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 378) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
