...
export const PROJECT_RUNTIME_PERSISTENCE_PLAN_V276_VERSION = 'v276-project-runtime-persistence-plan-v276';

export function createProjectRuntimePersistencePlanV276(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_PERSISTENCE_PLAN_V276_VERSION,
    versionNumber: 276,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimePersistencePlanV276(model = createProjectRuntimePersistencePlanV276()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_PERSISTENCE_PLAN_V276_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 276) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 276) errors.push('Runtime step invalid.');
