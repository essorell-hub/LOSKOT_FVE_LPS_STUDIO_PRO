...
export const LPS_SPD_FUNCTIONAL_HVI_PLANNER_V364_VERSION = 'v364-lps-spd-functional-hvi-planner-v364';

export function createLpsSpdFunctionalHviPlannerV364(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_HVI_PLANNER_V364_VERSION,
    versionNumber: 364,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'HviPlanner',
    status: 'V364_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalHviPlannerV364(model = createLpsSpdFunctionalHviPlannerV364()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_HVI_PLANNER_V364_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 364) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 364) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
