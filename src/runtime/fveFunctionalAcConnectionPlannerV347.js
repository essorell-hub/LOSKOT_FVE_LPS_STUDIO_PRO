...
export const FVE_FUNCTIONAL_AC_CONNECTION_PLANNER_V347_VERSION = 'v347-fve-functional-ac-connection-planner-v347';

export function createFveFunctionalAcConnectionPlannerV347(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_AC_CONNECTION_PLANNER_V347_VERSION,
    versionNumber: 347,
    phaseId: 'V341-V360',
    phaseTitle: 'FVE functional core',
    functionalDomain: 'fve-functional',
    functionalItem: 'AcConnectionPlanner',
    status: 'V347_READY',
    routeKey,
    project: {
...
export function validateFveFunctionalAcConnectionPlannerV347(model = createFveFunctionalAcConnectionPlannerV347()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_AC_CONNECTION_PLANNER_V347_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 347) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 347) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
