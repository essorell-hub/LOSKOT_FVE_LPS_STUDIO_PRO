...
export const LPS_SPD_RUNTIME_DC_SPD_MODEL_V311_VERSION = 'v311-lps-spd-runtime-dc-spd-model-v311';

export function createLpsSpdRuntimeDcSpdModelV311(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_DC_SPD_MODEL_V311_VERSION,
    versionNumber: 311,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeDcSpdModelV311(model = createLpsSpdRuntimeDcSpdModelV311()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_DC_SPD_MODEL_V311_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 311) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 311) errors.push('Runtime step invalid.');
