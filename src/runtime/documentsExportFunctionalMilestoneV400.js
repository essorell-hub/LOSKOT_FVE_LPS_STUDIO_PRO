...
export const DOCUMENTS_EXPORT_FUNCTIONAL_MILESTONE_V400_VERSION = 'v400-documents-export-functional-milestone-v400';

export function createDocumentsExportFunctionalMilestoneV400(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_MILESTONE_V400_VERSION,
    versionNumber: 400,
    phaseId: 'V381-V400',
    phaseTitle: 'documents export functional core',
    functionalDomain: 'documents-export-functional',
    functionalItem: 'Milestone',
    status: 'READY_V381_V400',
    routeKey,
    project: {
...
export function validateDocumentsExportFunctionalMilestoneV400(model = createDocumentsExportFunctionalMilestoneV400()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_MILESTONE_V400_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 400) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 400) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
