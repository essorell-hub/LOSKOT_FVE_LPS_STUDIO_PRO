...
export const DOCUMENTS_EXPORT_RUNTIME_QA_REPORT_MODEL_V330_VERSION = 'v330-documents-export-runtime-qa-report-model-v330';

export function createDocumentsExportRuntimeQaReportModelV330(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_QA_REPORT_MODEL_V330_VERSION,
    versionNumber: 330,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeQaReportModelV330(model = createDocumentsExportRuntimeQaReportModelV330()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_QA_REPORT_MODEL_V330_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 330) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 330) errors.push('Runtime step invalid.');
