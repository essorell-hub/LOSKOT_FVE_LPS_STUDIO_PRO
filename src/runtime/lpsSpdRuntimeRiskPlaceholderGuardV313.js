...
export const LPS_SPD_RUNTIME_RISK_PLACEHOLDER_GUARD_V313_VERSION = 'v313-lps-spd-runtime-risk-placeholder-guard-v313';

export function createLpsSpdRuntimeRiskPlaceholderGuardV313(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_RISK_PLACEHOLDER_GUARD_V313_VERSION,
    versionNumber: 313,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeRiskPlaceholderGuardV313(model = createLpsSpdRuntimeRiskPlaceholderGuardV313()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_RISK_PLACEHOLDER_GUARD_V313_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 313) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 313) errors.push('Runtime step invalid.');
