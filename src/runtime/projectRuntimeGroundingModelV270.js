...
export const PROJECT_RUNTIME_GROUNDING_MODEL_V270_VERSION = 'v270-project-runtime-grounding-model-v270';

export function createProjectRuntimeGroundingModelV270(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_GROUNDING_MODEL_V270_VERSION,
    versionNumber: 270,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeGroundingModelV270(model = createProjectRuntimeGroundingModelV270()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_GROUNDING_MODEL_V270_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 270) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 270) errors.push('Runtime step invalid.');
