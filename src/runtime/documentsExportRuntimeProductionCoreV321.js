...
export const DOCUMENTS_EXPORT_RUNTIME_PRODUCTION_CORE_V321_VERSION = 'v321-documents-export-runtime-production-core-v321';

export function createDocumentsExportRuntimeProductionCoreV321(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_PRODUCTION_CORE_V321_VERSION,
    versionNumber: 321,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeProductionCoreV321(model = createDocumentsExportRuntimeProductionCoreV321()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_PRODUCTION_CORE_V321_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 321) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 321) errors.push('Runtime step invalid.');
