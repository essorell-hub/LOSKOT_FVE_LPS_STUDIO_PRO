...
export const V68_FVE_UI_CALCULATION_BINDING_VERSION = "v68-fve-ui-calculation-binding";

const MODULE_NAME = "fveUiCalculationBinding";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V68_FVE_UI_CALCULATION_BINDING_VERSION,
...
export function createFveUiCalculationBinding(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V68_FVE_UI_CALCULATION_BINDING_VERSION,
      purpose: "Napojení FVE výpočtového enginu do UI workflow, inspectoru a QA.",
      classicProUnchanged: true
    });
  }
...
export function safeFveUiCalculationBinding(options = {}) {
  try {
    return createFveUiCalculationBinding(options);
  } catch (error) {
    return {
      version: V68_FVE_UI_CALCULATION_BINDING_VERSION,
      classicProUnchanged: true,
