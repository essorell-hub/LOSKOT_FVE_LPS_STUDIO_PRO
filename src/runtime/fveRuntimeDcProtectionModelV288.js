...
export const FVE_RUNTIME_DC_PROTECTION_MODEL_V288_VERSION = 'v288-fve-runtime-dc-protection-model-v288';

export function createFveRuntimeDcProtectionModelV288(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_DC_PROTECTION_MODEL_V288_VERSION,
    versionNumber: 288,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeDcProtectionModelV288(model = createFveRuntimeDcProtectionModelV288()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_DC_PROTECTION_MODEL_V288_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 288) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 288) errors.push('Runtime step invalid.');
