...
export const LPS_SPD_FUNCTIONAL_QA_ENGINE_V375_VERSION = 'v375-lps-spd-functional-qa-engine-v375';

export function createLpsSpdFunctionalQaEngineV375(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_QA_ENGINE_V375_VERSION,
    versionNumber: 375,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'QaEngine',
    status: 'V375_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalQaEngineV375(model = createLpsSpdFunctionalQaEngineV375()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_QA_ENGINE_V375_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 375) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 375) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
