...
export const PROJECT_RUNTIME_ELECTRICAL_MODEL_V267_VERSION = 'v267-project-runtime-electrical-model-v267';

export function createProjectRuntimeElectricalModelV267(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_ELECTRICAL_MODEL_V267_VERSION,
    versionNumber: 267,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeElectricalModelV267(model = createProjectRuntimeElectricalModelV267()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_ELECTRICAL_MODEL_V267_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 267) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 267) errors.push('Runtime step invalid.');
