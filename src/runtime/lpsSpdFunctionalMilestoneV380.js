...
export const LPS_SPD_FUNCTIONAL_MILESTONE_V380_VERSION = 'v380-lps-spd-functional-milestone-v380';

export function createLpsSpdFunctionalMilestoneV380(options = {}) {
...
export function validateLpsSpdFunctionalMilestoneV380(model = createLpsSpdFunctionalMilestoneV380()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_MILESTONE_V380_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 380) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 380) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
