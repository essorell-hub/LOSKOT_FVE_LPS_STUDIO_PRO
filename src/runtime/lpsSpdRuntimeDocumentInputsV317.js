...
export const LPS_SPD_RUNTIME_DOCUMENT_INPUTS_V317_VERSION = 'v317-lps-spd-runtime-document-inputs-v317';

export function createLpsSpdRuntimeDocumentInputsV317(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_DOCUMENT_INPUTS_V317_VERSION,
    versionNumber: 317,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeDocumentInputsV317(model = createLpsSpdRuntimeDocumentInputsV317()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_DOCUMENT_INPUTS_V317_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 317) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 317) errors.push('Runtime step invalid.');
