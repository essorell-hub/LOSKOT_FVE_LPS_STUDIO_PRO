...
export const QA_WORKFLOW_FUNCTIONAL_DESIGN_CORE_V401_VERSION = 'v401-qa-workflow-functional-design-core-v401';

export function createQaWorkflowFunctionalDesignCoreV401(options = {}) {
  ...
  const model = {
    modelVersion: QA_WORKFLOW_FUNCTIONAL_DESIGN_CORE_V401_VERSION,
    versionNumber: 401,
    phaseId: 'V401-V420',
...
export function validateQaWorkflowFunctionalDesignCoreV401(model = createQaWorkflowFunctionalDesignCoreV401()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_DESIGN_CORE_V401_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 401) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 401) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
