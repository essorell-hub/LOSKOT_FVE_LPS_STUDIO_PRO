...
export const FVE_FUNCTIONAL_DC_ROUTE_PLANNER_V345_VERSION = 'v345-fve-functional-dc-route-planner-v345';

export function createFveFunctionalDcRoutePlannerV345(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_DC_ROUTE_PLANNER_V345_VERSION,
    versionNumber: 345,
    phaseId: 'V341-V360',
    phaseTitle: 'FVE functional core',
    functionalDomain: 'fve-functional',
    functionalItem: 'DcRoutePlanner',
    status: 'V345_READY',
    routeKey,
    project: {
...
export function validateFveFunctionalDcRoutePlannerV345(model = createFveFunctionalDcRoutePlannerV345()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_DC_ROUTE_PLANNER_V345_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 345) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 345) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
