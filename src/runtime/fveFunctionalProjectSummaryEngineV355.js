...
export const FVE_FUNCTIONAL_PROJECT_SUMMARY_ENGINE_V355_VERSION = 'v355-fve-functional-project-summary-engine-v355';

export function createFveFunctionalProjectSummaryEngineV355(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_PROJECT_SUMMARY_ENGINE_V355_VERSION,
    versionNumber: 355,
    phaseId: 'V341-V360',
    phaseTitle: 'FVE functional core',
    functionalDomain: 'fve-functional',
    functionalItem: 'ProjectSummaryEngine',
    status: 'V355_READY',
    routeKey,
    project: {
...
export function validateFveFunctionalProjectSummaryEngineV355(model = createFveFunctionalProjectSummaryEngineV355()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_PROJECT_SUMMARY_ENGINE_V355_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 355) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 355) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
