...
export const LPS_SPD_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V374_VERSION = 'v374-lps-spd-functional-document-input-engine-v374';

export function createLpsSpdFunctionalDocumentInputEngineV374(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V374_VERSION,
    versionNumber: 374,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'DocumentInputEngine',
    status: 'V374_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalDocumentInputEngineV374(model = createLpsSpdFunctionalDocumentInputEngineV374()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V374_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 374) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 374) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
