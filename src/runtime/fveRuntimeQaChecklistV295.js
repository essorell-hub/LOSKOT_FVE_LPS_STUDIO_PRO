...
export const FVE_RUNTIME_QA_CHECKLIST_V295_VERSION = 'v295-fve-runtime-qa-checklist-v295';

export function createFveRuntimeQaChecklistV295(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_QA_CHECKLIST_V295_VERSION,
    versionNumber: 295,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeQaChecklistV295(model = createFveRuntimeQaChecklistV295()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_QA_CHECKLIST_V295_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 295) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 295) errors.push('Runtime step invalid.');
