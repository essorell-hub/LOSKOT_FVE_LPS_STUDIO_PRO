...
export const QA_WORKFLOW_FUNCTIONAL_INTEGRATION_ADAPTER_V417_VERSION = 'v417-qa-workflow-functional-integration-adapter-v417';

export function createQaWorkflowFunctionalIntegrationAdapterV417(options = {}) {
  ...
  const model = {
    modelVersion: QA_WORKFLOW_FUNCTIONAL_INTEGRATION_ADAPTER_V417_VERSION,
    versionNumber: 417,
    phaseId: 'V401-V420',
    phaseTitle: 'QA workflow integration functional core',
    functionalDomain: 'qa-workflow-functional',
    functionalItem: 'IntegrationAdapter',
    status: 'V417_READY',
    routeKey,
    project: {
...
export function validateQaWorkflowFunctionalIntegrationAdapterV417(model = createQaWorkflowFunctionalIntegrationAdapterV417()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_INTEGRATION_ADAPTER_V417_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 417) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 417) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
