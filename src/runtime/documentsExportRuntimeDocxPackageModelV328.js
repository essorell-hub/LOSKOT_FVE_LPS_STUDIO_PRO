...
export const DOCUMENTS_EXPORT_RUNTIME_DOCX_PACKAGE_MODEL_V328_VERSION = 'v328-documents-export-runtime-docx-package-model-v328';

export function createDocumentsExportRuntimeDocxPackageModelV328(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_DOCX_PACKAGE_MODEL_V328_VERSION,
    versionNumber: 328,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeDocxPackageModelV328(model = createDocumentsExportRuntimeDocxPackageModelV328()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_DOCX_PACKAGE_MODEL_V328_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 328) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 328) errors.push('Runtime step invalid.');
