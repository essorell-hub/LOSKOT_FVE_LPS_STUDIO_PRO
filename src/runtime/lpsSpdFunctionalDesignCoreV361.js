...
export const LPS_SPD_FUNCTIONAL_DESIGN_CORE_V361_VERSION = 'v361-lps-spd-functional-design-core-v361';

export function createLpsSpdFunctionalDesignCoreV361(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_DESIGN_CORE_V361_VERSION,
    versionNumber: 361,
    phaseId: 'V361-V380',
...
export function validateLpsSpdFunctionalDesignCoreV361(model = createLpsSpdFunctionalDesignCoreV361()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_DESIGN_CORE_V361_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 361) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 361) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
