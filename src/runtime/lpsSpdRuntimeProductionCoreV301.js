...
export const LPS_SPD_RUNTIME_PRODUCTION_CORE_V301_VERSION = 'v301-lps-spd-runtime-production-core-v301';

export function createLpsSpdRuntimeProductionCoreV301(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_RUNTIME_PRODUCTION_CORE_V301_VERSION,
    versionNumber: 301,
    phaseId: 'V301-V320',
...
export function validateLpsSpdRuntimeProductionCoreV301(model = createLpsSpdRuntimeProductionCoreV301()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_RUNTIME_PRODUCTION_CORE_V301_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 301) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 301) errors.push('Runtime step invalid.');
