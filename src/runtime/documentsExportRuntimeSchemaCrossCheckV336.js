...
export const DOCUMENTS_EXPORT_RUNTIME_SCHEMA_CROSS_CHECK_V336_VERSION = 'v336-documents-export-runtime-schema-cross-check-v336';

export function createDocumentsExportRuntimeSchemaCrossCheckV336(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_SCHEMA_CROSS_CHECK_V336_VERSION,
    versionNumber: 336,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeSchemaCrossCheckV336(model = createDocumentsExportRuntimeSchemaCrossCheckV336()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_SCHEMA_CROSS_CHECK_V336_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 336) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 336) errors.push('Runtime step invalid.');
