...
export const FVE_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V352_VERSION = 'v352-fve-functional-document-input-engine-v352';

export function createFveFunctionalDocumentInputEngineV352(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V352_VERSION,
    versionNumber: 352,
    phaseId: 'V341-V360',
...
export function validateFveFunctionalDocumentInputEngineV352(model = createFveFunctionalDocumentInputEngineV352()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_DOCUMENT_INPUT_ENGINE_V352_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 352) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 352) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
