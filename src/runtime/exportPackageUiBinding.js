...
export const V71_EXPORT_PACKAGE_UI_BINDING_VERSION = "v71-export-package-ui-binding";

const MODULE_NAME = "exportPackageUiBinding";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V71_EXPORT_PACKAGE_UI_BINDING_VERSION,
...
export function createExportPackageUiBinding(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V71_EXPORT_PACKAGE_UI_BINDING_VERSION,
      purpose: "Napojení project package builderu do UI, manifestu a handoff checklistu.",
      classicProUnchanged: true
    });
  }
...
export function safeExportPackageUiBinding(options = {}) {
  try {
    return createExportPackageUiBinding(options);
  } catch (error) {
    return {
      version: V71_EXPORT_PACKAGE_UI_BINDING_VERSION,
      classicProUnchanged: true,
