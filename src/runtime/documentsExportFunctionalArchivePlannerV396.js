...
export const DOCUMENTS_EXPORT_FUNCTIONAL_ARCHIVE_PLANNER_V396_VERSION = 'v396-documents-export-functional-archive-planner-v396';

export function createDocumentsExportFunctionalArchivePlannerV396(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_ARCHIVE_PLANNER_V396_VERSION,
    versionNumber: 396,
    phaseId: 'V381-V400',
    phaseTitle: 'documents export functional core',
    functionalDomain: 'documents-export-functional',
    functionalItem: 'ArchivePlanner',
    status: 'V396_READY',
    routeKey,
    project: {
...
export function validateDocumentsExportFunctionalArchivePlannerV396(model = createDocumentsExportFunctionalArchivePlannerV396()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_ARCHIVE_PLANNER_V396_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 396) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 396) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
