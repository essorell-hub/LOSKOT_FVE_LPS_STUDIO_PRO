...
export const FVE_RUNTIME_STRING_PLAN_MODEL_V284_VERSION = 'v284-fve-runtime-string-plan-model-v284';

export function createFveRuntimeStringPlanModelV284(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_STRING_PLAN_MODEL_V284_VERSION,
    versionNumber: 284,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeStringPlanModelV284(model = createFveRuntimeStringPlanModelV284()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_STRING_PLAN_MODEL_V284_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 284) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 284) errors.push('Runtime step invalid.');
