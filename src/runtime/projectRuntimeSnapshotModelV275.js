...
export const PROJECT_RUNTIME_SNAPSHOT_MODEL_V275_VERSION = 'v275-project-runtime-snapshot-model-v275';

export function createProjectRuntimeSnapshotModelV275(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_SNAPSHOT_MODEL_V275_VERSION,
    versionNumber: 275,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeSnapshotModelV275(model = createProjectRuntimeSnapshotModelV275()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_SNAPSHOT_MODEL_V275_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 275) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 275) errors.push('Runtime step invalid.');
