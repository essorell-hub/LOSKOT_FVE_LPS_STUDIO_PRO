...
export const LPS_SPD_FUNCTIONAL_RISK_PLACEHOLDER_GUARD_V371_VERSION = 'v371-lps-spd-functional-risk-placeholder-guard-v371';

export function createLpsSpdFunctionalRiskPlaceholderGuardV371(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_RISK_PLACEHOLDER_GUARD_V371_VERSION,
    versionNumber: 371,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'RiskPlaceholderGuard',
    status: 'V371_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalRiskPlaceholderGuardV371(model = createLpsSpdFunctionalRiskPlaceholderGuardV371()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_RISK_PLACEHOLDER_GUARD_V371_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 371) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 371) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
