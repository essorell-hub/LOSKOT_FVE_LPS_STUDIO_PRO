...
export const QA_WORKFLOW_FUNCTIONAL_GRAPHICS_MANIFEST_V415_VERSION = 'v415-qa-workflow-functional-graphics-manifest-v415';

export function createQaWorkflowFunctionalGraphicsManifestV415(options = {}) {
  ...
  const model = {
    modelVersion: QA_WORKFLOW_FUNCTIONAL_GRAPHICS_MANIFEST_V415_VERSION,
    versionNumber: 415,
    phaseId: 'V401-V420',
    phaseTitle: 'QA workflow integration functional core',
    functionalDomain: 'qa-workflow-functional',
    functionalItem: 'GraphicsManifest',
    status: 'V415_READY',
    routeKey,
    project: {
...
export function validateQaWorkflowFunctionalGraphicsManifestV415(model = createQaWorkflowFunctionalGraphicsManifestV415()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_GRAPHICS_MANIFEST_V415_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 415) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 415) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
