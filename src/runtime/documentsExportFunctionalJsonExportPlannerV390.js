...
export const DOCUMENTS_EXPORT_FUNCTIONAL_JSON_EXPORT_PLANNER_V390_VERSION = 'v390-documents-export-functional-json-export-planner-v390';

export function createDocumentsExportFunctionalJsonExportPlannerV390(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_FUNCTIONAL_JSON_EXPORT_PLANNER_V390_VERSION,
    versionNumber: 390,
    phaseId: 'V381-V400',
    phaseTitle: 'documents export functional core',
    functionalDomain: 'documents-export-functional',
    functionalItem: 'JsonExportPlanner',
    status: 'V390_READY',
    routeKey,
    project: {
...
export function validateDocumentsExportFunctionalJsonExportPlannerV390(model = createDocumentsExportFunctionalJsonExportPlannerV390()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_FUNCTIONAL_JSON_EXPORT_PLANNER_V390_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 390) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 390) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
