...
export const LPS_SPD_FUNCTIONAL_EXPORT_ADAPTER_V377_VERSION = 'v377-lps-spd-functional-export-adapter-v377';

export function createLpsSpdFunctionalExportAdapterV377(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_EXPORT_ADAPTER_V377_VERSION,
    versionNumber: 377,
    phaseId: 'V361-V380',
...
export function validateLpsSpdFunctionalExportAdapterV377(model = createLpsSpdFunctionalExportAdapterV377()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_EXPORT_ADAPTER_V377_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 377) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 377) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
