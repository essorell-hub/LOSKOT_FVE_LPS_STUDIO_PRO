...
export const LPS_SPD_FUNCTIONAL_AC_SPD_PLANNER_V370_VERSION = 'v370-lps-spd-functional-ac-spd-planner-v370';

export function createLpsSpdFunctionalAcSpdPlannerV370(options = {}) {
  ...
  const model = {
    modelVersion: LPS_SPD_FUNCTIONAL_AC_SPD_PLANNER_V370_VERSION,
    versionNumber: 370,
    phaseId: 'V361-V380',
    phaseTitle: 'LPS SPD functional core',
    functionalDomain: 'lps-spd-functional',
    functionalItem: 'AcSpdPlanner',
    status: 'V370_READY',
    routeKey,
    project: {
...
export function validateLpsSpdFunctionalAcSpdPlannerV370(model = createLpsSpdFunctionalAcSpdPlannerV370()) {
  const errors = [];
  if (model.modelVersion !== LPS_SPD_FUNCTIONAL_AC_SPD_PLANNER_V370_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 370) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 370) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
