...
export const FVE_FUNCTIONAL_MILESTONE_V360_VERSION = 'v360-fve-functional-milestone-v360';

export function createFveFunctionalMilestoneV360(options = {}) {
...
export function validateFveFunctionalMilestoneV360(model = createFveFunctionalMilestoneV360()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_MILESTONE_V360_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 360) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 360) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
