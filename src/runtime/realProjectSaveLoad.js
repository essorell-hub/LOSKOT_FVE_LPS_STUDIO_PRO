...
export const V66_REAL_PROJECT_SAVE_LOAD_VERSION = "v66-real-project-save-load";

const MODULE_NAME = "realProjectSaveLoad";
...
function createResult(action, data = null, warnings = [], errors = []) {
  return createRuntimeResult({
    ...
    errors,
    meta: {
      moduleVersion: V66_REAL_PROJECT_SAVE_LOAD_VERSION,
...
export function createRealProjectSaveLoad(options = {}) {
  function getPurpose() {
    return createResult("getPurpose", {
      version: V66_REAL_PROJECT_SAVE_LOAD_VERSION,
      purpose: "Reálné save/load projektu nad jednotným projektovým modelem, JSON snapshoty a bezpečný restore.",
      classicProUnchanged: true
    });
  }
...
export function safeRealProjectSaveLoad(options = {}) {
  try {
    return createRealProjectSaveLoad(options);
  } catch (error) {
    return {
      version: V66_REAL_PROJECT_SAVE_LOAD_VERSION,
      classicProUnchanged: true,
