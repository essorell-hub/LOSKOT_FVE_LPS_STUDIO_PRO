...
export const DOCUMENTS_EXPORT_RUNTIME_DEVICE_LIST_MODEL_V325_VERSION = 'v325-documents-export-runtime-device-list-model-v325';

export function createDocumentsExportRuntimeDeviceListModelV325(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_DEVICE_LIST_MODEL_V325_VERSION,
    versionNumber: 325,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeDeviceListModelV325(model = createDocumentsExportRuntimeDeviceListModelV325()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_DEVICE_LIST_MODEL_V325_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 325) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 325) errors.push('Runtime step invalid.');
