...
export const FVE_RUNTIME_PRODUCTION_CORE_V281_VERSION = 'v281-fve-runtime-production-core-v281';

export function createFveRuntimeProductionCoreV281(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_PRODUCTION_CORE_V281_VERSION,
    versionNumber: 281,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeProductionCoreV281(model = createFveRuntimeProductionCoreV281()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_PRODUCTION_CORE_V281_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 281) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 281) errors.push('Runtime step invalid.');
