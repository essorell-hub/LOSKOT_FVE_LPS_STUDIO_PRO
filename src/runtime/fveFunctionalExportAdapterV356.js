...
export const FVE_FUNCTIONAL_EXPORT_ADAPTER_V356_VERSION = 'v356-fve-functional-export-adapter-v356';

export function createFveFunctionalExportAdapterV356(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_EXPORT_ADAPTER_V356_VERSION,
    versionNumber: 356,
    phaseId: 'V341-V360',
...
export function validateFveFunctionalExportAdapterV356(model = createFveFunctionalExportAdapterV356()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_EXPORT_ADAPTER_V356_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 356) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 356) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
