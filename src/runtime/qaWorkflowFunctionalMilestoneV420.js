...
export const QA_WORKFLOW_FUNCTIONAL_MILESTONE_V420_VERSION = 'v420-qa-workflow-functional-milestone-v420';

export function createQaWorkflowFunctionalMilestoneV420(options = {}) {
...
export function validateQaWorkflowFunctionalMilestoneV420(model = createQaWorkflowFunctionalMilestoneV420()) {
  const errors = [];
  if (model.modelVersion !== QA_WORKFLOW_FUNCTIONAL_MILESTONE_V420_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 420) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 420) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
