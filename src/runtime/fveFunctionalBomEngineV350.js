...
export const FVE_FUNCTIONAL_BOM_ENGINE_V350_VERSION = 'v350-fve-functional-bom-engine-v350';

export function createFveFunctionalBomEngineV350(options = {}) {
  ...
  const model = {
    modelVersion: FVE_FUNCTIONAL_BOM_ENGINE_V350_VERSION,
    versionNumber: 350,
    phaseId: 'V341-V360',
...
export function validateFveFunctionalBomEngineV350(model = createFveFunctionalBomEngineV350()) {
  const errors = [];
  if (model.modelVersion !== FVE_FUNCTIONAL_BOM_ENGINE_V350_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 350) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 350) errors.push('Runtime step invalid.');
  return {
    ok: errors.length === 0,
