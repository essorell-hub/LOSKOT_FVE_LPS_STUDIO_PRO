...
export const PROJECT_RUNTIME_ROOF_MODEL_V266_VERSION = 'v266-project-runtime-roof-model-v266';

export function createProjectRuntimeRoofModelV266(options = {}) {
  ...
  const model = {
    modelVersion: PROJECT_RUNTIME_ROOF_MODEL_V266_VERSION,
    versionNumber: 266,
    phaseId: 'V261-V280',
...
export function validateProjectRuntimeRoofModelV266(model = createProjectRuntimeRoofModelV266()) {
  const errors = [];
  if (model.modelVersion !== PROJECT_RUNTIME_ROOF_MODEL_V266_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 266) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 266) errors.push('Runtime step invalid.');
