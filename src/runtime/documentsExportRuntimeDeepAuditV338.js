...
export const DOCUMENTS_EXPORT_RUNTIME_DEEP_AUDIT_V338_VERSION = 'v338-documents-export-runtime-deep-audit-v338';

export function createDocumentsExportRuntimeDeepAuditV338(options = {}) {
  ...
  const model = {
    modelVersion: DOCUMENTS_EXPORT_RUNTIME_DEEP_AUDIT_V338_VERSION,
    versionNumber: 338,
    phaseId: 'V321-V340',
...
export function validateDocumentsExportRuntimeDeepAuditV338(model = createDocumentsExportRuntimeDeepAuditV338()) {
  const errors = [];
  if (model.modelVersion !== DOCUMENTS_EXPORT_RUNTIME_DEEP_AUDIT_V338_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 338) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 338) errors.push('Runtime step invalid.');
