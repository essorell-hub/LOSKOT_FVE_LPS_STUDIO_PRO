...
export const FVE_RUNTIME_RELEASE_GATE_V298_VERSION = 'v298-fve-runtime-release-gate-v298';

export function createFveRuntimeReleaseGateV298(options = {}) {
  ...
  const model = {
    modelVersion: FVE_RUNTIME_RELEASE_GATE_V298_VERSION,
    versionNumber: 298,
    phaseId: 'V281-V300',
...
export function validateFveRuntimeReleaseGateV298(model = createFveRuntimeReleaseGateV298()) {
  const errors = [];
  if (model.modelVersion !== FVE_RUNTIME_RELEASE_GATE_V298_VERSION) errors.push('Unexpected model version.');
  if (model.versionNumber !== 298) errors.push('Unexpected version number.');
  ...
  if (!model.metrics || model.metrics.runtimeStep !== 298) errors.push('Runtime step invalid.');
