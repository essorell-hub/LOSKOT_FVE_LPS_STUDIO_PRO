...
echo "===== LOSKOT VERIFY MEGA PACK F ====="
...
for f in src/runtime/v91_v95_release_hardening_diagnostics_backup/*.js; do
  node --check "$f"
done

node --check tests/runtime/v91_v95_release_hardening_smoke_test.js
node tests/runtime/v91_v95_release_hardening_smoke_test.js

if grep -RInE "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties|AssertionError|command failed" src/runtime/v91_v95_release_hardening_diagnostics_backup tests/runtime/v91_v95_release_hardening_smoke_test.js 2>/dev/null; then
