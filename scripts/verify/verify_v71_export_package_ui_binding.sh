...
LOG_DIR="dist/verify"
LOG_FILE="$LOG_DIR/v71_verify.log"
mkdir -p "$LOG_DIR"
: > "$LOG_FILE"
...
say "===== VERIFY V71 EXPORT PACKAGE UI BINDING START ====="
...
run_check node --check src/runtime/exportPackageUiBinding.js
run_check node --check tests/runtime/v71_export_package_ui_binding_smoke_test.js
run_check node tests/runtime/v71_export_package_ui_binding_smoke_test.js
...
if grep -Ein "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties" "$LOG_FILE" > "$LOG_DIR/v71_strict_errors.log"; then
  say "ERROR: strict log scan found errors"
  cat "$LOG_DIR/v71_strict_errors.log" | tee -a "$LOG_FILE"
  FAIL=1
else
...
say "LOG=$LOG_FILE"
say "========================="
say "===== VERIFY V71 EXPORT PACKAGE UI BINDING END ====="
exit "$FAIL"
