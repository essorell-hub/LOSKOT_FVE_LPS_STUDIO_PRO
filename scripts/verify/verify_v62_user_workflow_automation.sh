...
set -u
LOG_DIR="dist/verify"
LOG_FILE="$LOG_DIR/v62_verify.log"
mkdir -p "$LOG_DIR"
: > "$LOG_FILE"
FAIL=0
DIFFCHECK=1
...
say "===== VERIFY V62 USER WORKFLOW AUTOMATION START ====="
required_files=(
...
run_check node --check src/runtime/userWorkflowAutomation.js
run_check node --check tests/runtime/v62_user_workflow_automation_smoke_test.js
run_check node tests/runtime/v62_user_workflow_automation_smoke_test.js
...
if grep -Ein "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties" "$LOG_FILE" > "$LOG_DIR/v62_strict_errors.log"; then
  say "ERROR: strict log scan found errors"
  cat "$LOG_DIR/v62_strict_errors.log" | tee -a "$LOG_FILE"
  FAIL=1
else
...
say "LOG=$LOG_FILE"
say "========================="
say "===== VERIFY V62 USER WORKFLOW AUTOMATION END ====="
exit "$FAIL"
