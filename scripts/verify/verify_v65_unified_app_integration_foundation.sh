...
LOG_DIR="dist/verify"
LOG_FILE="$LOG_DIR/v65_verify.log"
mkdir -p "$LOG_DIR"
: > "$LOG_FILE"
...
say "===== VERIFY V65 UNIFIED APP INTEGRATION FOUNDATION START ====="
...
run_check node --check src/runtime/unifiedAppIntegration.js
run_check node --check tests/runtime/v65_unified_app_integration_foundation_smoke_test.js
run_check node tests/runtime/v65_unified_app_integration_foundation_smoke_test.js
...
if grep -Ein "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties" "$LOG_FILE" > "$LOG_DIR/v65_strict_errors.log"; then
  say "ERROR: strict log scan found errors"
  cat "$LOG_DIR/v65_strict_errors.log" | tee -a "$LOG_FILE"
  FAIL=1
else
...
say "LOG=$LOG_FILE"
say "========================="
say "===== VERIFY V65 UNIFIED APP INTEGRATION FOUNDATION END ====="
exit "$FAIL"
