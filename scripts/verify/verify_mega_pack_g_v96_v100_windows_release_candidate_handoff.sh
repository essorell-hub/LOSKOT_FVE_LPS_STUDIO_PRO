...
echo "===== LOSKOT VERIFY MEGA PACK G ====="
...
for f in src/runtime/v96_v100_windows_release_candidate_handoff/*.js; do
  node --check "$f"
done

node --check tests/runtime/v96_v100_windows_release_candidate_smoke_test.js
node tests/runtime/v96_v100_windows_release_candidate_smoke_test.js

if grep -RInE "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties|AssertionError|command failed" src/runtime/v96_v100_windows_release_candidate_handoff tests/runtime/v96_v100_windows_release_candidate_smoke_test.js 2>/dev/null; then
