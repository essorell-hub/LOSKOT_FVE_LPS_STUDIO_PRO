...
LOG_DIR="dist/verify"
LOG_FILE="$LOG_DIR/v68_verify.log"
mkdir -p "$LOG_DIR"
: > "$LOG_FILE"
...
say "===== VERIFY V68 FVE UI CALCULATION BINDING START ====="
...
run_check node --check src/runtime/fveUiCalculationBinding.js
run_check node --check tests/runtime/v68_fve_ui_calculation_binding_smoke_test.js
run_check node tests/runtime/v68_fve_ui_calculation_binding_smoke_test.js
...
if grep -Ein "TypeError|ReferenceError|SyntaxError|FAILED|test failed|is not a function|is not defined|ERR_MODULE_NOT_FOUND|Cannot read properties" "$LOG_FILE" > "$LOG_DIR/v68_strict_errors.log"; then
  say "ERROR: strict log scan found errors"
  cat "$LOG_DIR/v68_strict_errors.log" | tee -a "$LOG_FILE"
  FAIL=1
else
...
say "LOG=$LOG_FILE"
say "========================="
say "===== VERIFY V68 FVE UI CALCULATION BINDING END ====="
exit "$FAIL"
